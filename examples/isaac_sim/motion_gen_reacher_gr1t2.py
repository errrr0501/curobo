...
parser.add_argument("--robot", type=str, default="gr1t2.yml", help="robot configuration to load")
parser.add_argument(
    "--external_asset_path",
    type=str,
    default=None,
...
############################################################
from omni.isaac.core.utils import stage as stage_isaac
...
ROBOT_BASE = np.array([0.0, 0.0, 0.953])
...
def main():
    ...
    ######################################
    # stage_isaac.add_reference_to_stage(GLUE_MACHINE_USD_PATH, BACKGROUND_STAGE_PATH)
    # prims.create_prim(
    ...
    robot, robot_prim_path = add_robot_to_scene(robot_cfg, my_world, position=ROBOT_BASE)
    ...
    while simulation_app.is_running():
        ...
        if (
            ...
            # prim = stage.GetPrimAtPath("/GR1_T2/l_hand_roll")
            ...
            # quaternion_tuple = (quat.GetReal(), quat.GetImaginary()[0],
            #                     quat.GetImaginary()[1], quat.GetImaginary()[2])
            # print(translation)
            # print(quaternion_tuple)


            # ee_translation_goal = np.array([-1.4146567650641373e-7, 0.2306157648563385, 0.8575135469436646])
            # ee_orientation_teleop_goal = np.array([0.9999999999916961,
            #                                        -3.898068415046693e-06, 
            #                                       -5.960365063125767e-08, 
            #                                       -1.1871546915821659e-06, 
            #                                       ])
            ####################################
            # ee_orientation_teleop_goal = np.array([1.0, 0.0, 0.0, 0.0])
